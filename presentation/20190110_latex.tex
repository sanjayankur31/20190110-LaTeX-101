...
\begin{frame}[fragile]{Generating the document: II}
  \begin{itemize}
    \item The default output from \LaTeX{} is \href{https://en.wikipedia.org/wiki/Device_independent_file_format}{DVI} (Device independent file format).
      \pause{}
...
right angled triangle respectively.
  ...
  \end{itemize}
\end{frame}
\begin{frame}[fragile]{More text structures}
  \begin{itemize}
    \item tables,
    \item figures, subfigures,
    \item headers, footers,
    \item source code snippets,
    \item algorithms,
    \item theorems,
    \item timelines,
    \item margin comments,
    \item indexes, glossaries,
    \item \ldots
  \end{itemize}
\end{frame}
...
\begin{frame}[fragile]{Citations and referencing with BibTeX II}
  \begin{itemize}
    \item An example BibTeX entry:
  \end{itemize}
  \begin{figure}[h]
    \centering
    \begin{minted}[fontsize=\footnotesize]{bibtex}
% Save as mybib.bib in the same directory
% as the document
@Misc{RedHat2008,
author = {RedHat},
...
to promoting the Free/Open source philosophy.
...
LaTeX Warning: There were undefined references.
  ...
  \begin{figure}[h]
    \centering
    \includegraphics[width=\textwidth,keepaspectratio]{20190110-latex-101-bibtex.png}
  \end{figure}
\end{frame}
...
$ pdflatex example-doc && bibtex example-doc \
  ...
  \pause{}
  \begin{itemize}
    \item Most IDEs permit setting up the complete chain of necessary commands.
    \item Terminal users, try \href{https://mg.readthedocs.io/latexmk.html}{Latexmk} (Yes, it's available in Fedora).
  \end{itemize}
\end{frame}
...
\begin{frame}[c]{Using Git/Github}
  \begin{itemize}
    ...
    \item Use the power of \href{https://github.com/sanjayankur31/20190110-LaTeX-101/commits/master}{version control} (instead of \href{http://phdcomics.com/comics/archive.php?comicid=1531}{this}!)
    \item Can follow the standard \alert{pull/merge} model.
      \pause{}
    \item A few tips:
      \begin{itemize}
        \item Write \alert{each sentence on a new line}: this helps git to merge easily, since git looks fir differences between lines.
        \item Break the main text into \alert{smaller files} using \texttt{include} or \texttt{input} commands\footnotemark.
      \end{itemize}
  \end{itemize}
...
\section{Epilogue}
\begin{frame}[c]{Common issues}
  \begin{itemize}
    \item It isn't WYSIWYG (what you see is what you get, like Word)
      \begin{itemize}
        \item That is the whole point: \alert{stop thinking about how it'll look}. \LaTeX{} will take care of that. Focus on writing the text!
      \end{itemize}
      \pause{}
    \item Command writing is tedious!
      \begin{itemize}
        \item Use an IDE that provides \alert{completion}. If you write code, the same IDE that you use for programming will most probably also work for writing \LaTeX{}.
      \end{itemize}
      \pause{}
    \item I don't know Git:
      \begin{itemize}
        \item Use \alert{Overleaf}: it provides a web interface where you don't need to use Git explicitly. (but learn Git if possible)
      \end{itemize}
      \pause{}
    \item Error messages are \alert{hard to understand}:
      \begin{itemize}
        \item Remove bits of the text to isolate what is causing the error. If that doesn't work, \href{https://docs.fedoraproject.org/en-US/neurofedora/latex/\#_getting_help}{ask for help}.
      \end{itemize}
  \end{itemize}
\end{frame}
